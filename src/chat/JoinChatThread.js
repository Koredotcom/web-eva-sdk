...
const JoinChatThread = async (props) => {
	...
	const Res = await store.dispatch(
		getSearchHistory({ boardId: props.boardId, params })
	);
	...
	const afterApiCallSuccess = async (data, args) => {
		...
		for (const q of historyData) {
			...
			if (q?.viewType === "threadView") {
				...
				const botChatData = await store.dispatch(
					getSearchHistory({ boardId: q?.boardId, params })
				);
				if (botChatData?.payload?.history?.length) {
					const orderedBotChatData = orderBy(
						botChatData.payload.history,
						"msgNo",
						"asc"
					);
					...
					orderedBotChatData.map((detail) => {
						if (detail?.templateType === "bot_template") {
							if (state?.enableKoreBotSDK) {
								detail.template_html =
									BotConversation().generateHTMLforBotTemplate(
										detail
									);
							}
						}
					});
					obj.botConversation = keyBy(
						orderedBotChatData,
						"messageId"
					);
			...
			if (q?.postType === "follow-up" && !q?.context) {
				...
				let parentQuestion = data?.history?.find(
					(value) =>
						value?.context && q?.pId === value?.context?.sessionId
				);
