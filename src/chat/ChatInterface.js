...
const ChatInterface = (props) => {
	...
	const initiateChatConversationAction = async (arg) => {
		...
		if (arg?.createIssue) {
			if (arg?.from === "gptAgent") {
				...
				params.reqId = getCidByMessageId(
					state.questions,
					payload?.messageId
				);
				replaceExistingQsn = true;
	  ...
	  */
		if (
			...
			const newRes = await store.dispatch(
				advanceSearch({
					params,
					payload,
					userId: state.profile.data.id,
	...
	const contentStreaming = (detail) => {
		...
		if (detail?.data?.status === "in-progress") {
			if (detail?.data?.templateType === "multi_responses") {
				...
				question.responses[resIndex].answer = question?.responses?.[
					resIndex
				]?.answer?.concat(detail?.data?.chunk);
	...
	const options = (_options) => {
		...
		store.dispatch(
			setChatInterfaceOptions({ ...chatOptions, ..._options })
		);
	};
