...
const globalSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		handleAsyncActions(
			...
			"allAgents",
			(state, action) => {
				let enabledAgents = action.payload.agents.filter(
					(a) => !!a?.enabled
				);
        ...
        */
				if (action?.meta?.arg?.params?.from === "botAgent") {
					...
					const currentBotAgentQuestion = Object.values(
						questions
					).find((ques) => ques.reqId === botReqId);
					if (currentBotAgentQuestion) {
						/*for history question we need to rely on id */
						if (currentBotAgentQuestion?.historicalData) {
							if (
								questions?.[
									currentBotAgentQuestion?.id
								]?.hasOwnProperty("botConversation")
							) {
								questions[
									currentBotAgentQuestion?.id
								].botConversation[action?.payload?.messageId] =
									action?.payload;
							}
						} else {
							if (
								questions?.[
									currentBotAgentQuestion?.reqId
								]?.hasOwnProperty("botConversation")
							) {
								questions[
									currentBotAgentQuestion?.reqId
		...
		handleAsyncActions(
			...
			(state, action) => {
				...
				if (action?.meta?.arg?.initialData) {
					...
					if (action?.meta?.arg?.deleteLoader) {
						allHistory = state.historyRes?.data?.boards?.filter(
							(board) => board?.loading !== true
						);
					}
					let _allHistory = uniqBy(
						concat(allHistory, state.historyRes?.data?.boards),
						"id"
					);
					allHistory = orderBy(_allHistory, ["sortTime"], ["desc"]);
				}
				...
				state.AllHistory.hasMore =
					state.historyRes?.data?.moreAvailable;
				// }
			}
		);
		handleAsyncActions(
			...
			(state, action) => {
				...
				if (action?.meta?.arg?.loadmore) {
					let AllrecentFiles = cloneDeep(
						state.AllrecentFiles?.data?.files
					);
					AllrecentFiles = uniqBy(
						concat(
							AllrecentFiles,
							state.recentFilesRes?.data?.files
						),
						"id"
					);
