...
const FileUpload = (props) => {
	...
	const uploadFile = (event) => {
		...
		if (files && files.length) {
			...
			if (state?.selectedContext?.data?.sessionId) {
				...
				_selectedContext.data.sessionId =
					state?.selectedContext?.data?.sessionId;
				_selectedContext.data.quickactions =
					state?.selectedContext?.data?.quickactions;
			}
						...
						if (allSources?.length !== selectedSources?.length) {
							...
							allSources = allSources.filter((source) =>
								selectedSources.some(
									(selected) => selected.uID === source.uID
								)
							);
						}
						//If there are no sources to add, no searchSession call is to be made.
						let action = state?.selectedContext?.data?.sessionId
							? "update"
							: "add";
						allSources?.length &&
							uploadFiles({ attachments: allSources, action });
					}
				});
			}
		}
	...
	const uploadFileInitial = (file, allSources, onComplete) => {
		...
		let allowedFileSize = Math.round(
			state.maxAllowedFileSize / Math.pow(1024, 2)
		);
		...
		if (localSize > allowedFileSize) {
			...
			let remainingFiles = state.selectedContext.data.sources.filter(
				(f) => f.uID !== file.uID
			);
	...
	const setAttachmentContext = (args) => {
		...
		let isAgentSetAsSource = _agents.find(
			(ag) => ag.id === selectedContext?.data?.sources?.[0]?.source
		);
		let sourceType = isAgentSetAsSource ? "agent" : null;
		let discardPrevSession = false;
		if (
			sourceType === "agent" ||
			selectedContext?.data?.sources?.[0]?.isAgent
		) {
	...
	const askFollowup = (args) => {
		...
		const messageId =
			msgType === "followup" ? args?.parentMessageId : args?.messageId;
		...
		if (
			args?.viewType === "table" &&
			_selectedContext?.hasOwnProperty("sessionId")
		) {
			sessionItemHandler({
			...
			let _agents = cloneDeep(enabledAgents);
			let isAgentSetAsSource = _agents.find(
				(ag) => ag.id === args?.sources?.[0]?.source
			);
			let sourceType = isAgentSetAsSource ? "agent" : null;
			let obj = {
